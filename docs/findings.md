...
│                                                  │
│   ┌───────────────┐   ┌────────────────────┐    │
│   │ CandleCollector│──▶│ SignalGeneratorV2  │    │
│   │ (DOM 수집)    │   │ (RSI/EMA V2)       │    │
│   └───────────────┘   └─────────┬──────────┘    │
│                                  │               │
│   ┌───────────────┐              ▼               │
...
### V2 Trading Flow
1. **CandleCollector**: DOM에서 가격 변화 감지 → 1분 캔들 생성
2. **SignalGeneratorV2**: 캔들 데이터로 RSI/EMA V2 신호 생성
3. **PayoutMonitor**: 92%+ 페이아웃 자산 필터링
4. **TradeExecutor**: 조건 충족 시 거래 실행

### 안전 장치
- `onlyRSI: true` - Forward Test 검증된 RSI만 사용
- `minPayout: 92` - 손익분기점 고려
- `maxConsecutiveLosses: 5` - 연속 손실 제한
- `maxDrawdown: 20` - 최대 손실률 제한
- Demo 모드 필수 체크
