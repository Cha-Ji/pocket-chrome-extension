// ============================================================
// Forward Test V2 - High Win Rate Strategy
// ============================================================
// 개선 사항:
// 1. SignalGeneratorV2 사용 (고승률 전략)
// 2. 추세 필터링 적용
// 3. 신뢰도 기반 필터링
// 4. 상세 분석 리포트
// ============================================================

import { SignalGeneratorV2 } from '../src/lib/signals/signal-generator-v2'
import { fetchCandles } from '../src/lib/signals/signal-generator'
import { detectRegime } from '../src/lib/signals/strategies'
import * as fs from 'fs'

interface ForwardTestResult {
  timestamp: string
  symbol: string
  direction: 'CALL' | 'PUT'
  strategy: string
  regime: string
  confidence: number
  entryPrice: number
  exitPrice: number
  result: 'WIN' | 'LOSS' | 'TIE'
  profit: number
}

interface ForwardTestReport {
  testInfo: {
    startTime: string
    endTime: string
    durationMinutes: number
    symbol: string
    initialBalance: number
    riskPerTrade: string
    version: string
  }
  summary: {
    totalTrades: number
    wins: number
    losses: number
    winRate: string
    netProfit: string
    finalBalance: string
    avgConfidence: string
  }
  byRegime: Record<string, { trades: number; wins: number; losses: number; winRate: string }>
  byStrategy: Record<string, { trades: number; wins: number; losses: number; winRate: string }>
  trades: ForwardTestResult[]
}

async function runForwardTestV2(durationMs: number = 180000) {
  console.log('🚀 Forward Test V2 Started')
  console.log('🎯 Using High Win-Rate Strategies')
  console.log(`Duration: ${durationMs / 60000} minutes`)
  console.log('═'.repeat(60))

  // V2 신호 생성기 사용
  const generator = new SignalGeneratorV2({
    minConfidence: 0.6,
    useTrendFilter: true,
    minVotesForSignal: 2,
    highWinRateConfig: {
      rsiPeriod: 7,
      rsiOversold: 25,
      rsiOverbought: 75,
    }
  })

  const symbol = 'BTCUSDT'
  const initialBalance = 1000
  const riskPercent = 1
  let balance = initialBalance
  const trades: ForwardTestResult[] = []
  const startTime = Date.now()

  // Load initial history
  const history = await fetchCandles(symbol, '1m', 100)
  generator.setHistory(symbol, history)
  console.log(`📊 Loaded ${history.length} candles\n`)

  const checkInterval = 10000 // Check every 10 seconds
  let lastCheck = 0
  let skippedSignals = 0

  while (Date.now() - startTime < durationMs) {
    const now = Date.now()

    if (now - lastCheck < checkInterval) {
      await new Promise(r => setTimeout(r, 1000))
      continue
    }
    lastCheck = now

    try {
      // Fetch latest candles
      const candles = await fetchCandles(symbol, '1m', 5)
      const latestCandle = candles[candles.length - 1]

      // Check regime
      const allCandles = await fetchCandles(symbol, '1m', 100)
      const regimeInfo = detectRegime(allCandles)

      // Add candle and check for signal
      const signal = generator.addCandle(symbol, latestCandle)

      const time = new Date().toLocaleTimeString()
      const stats = generator.getStats()
      console.log(`[${time}] ${symbol} $${latestCandle.close.toFixed(2)} | ${regimeInfo.regime} (ADX: ${regimeInfo.adx.toFixed(1)}) | Signals: ${stats.signalsGenerated}, Filtered: ${stats.signalsFiltered}`)

      if (signal) {
        const amount = (balance * riskPercent) / 100
        console.log(`\n🎯 SIGNAL: ${signal.direction}`)
        console.log(`   Strategy: ${signal.strategy}`)
        console.log(`   Confidence: ${(signal.confidence * 100).toFixed(0)}%`)
        console.log(`   Entry: $${signal.entryPrice.toFixed(2)}`)
        console.log(`   Amount: $${amount.toFixed(2)} (${riskPercent}% of $${balance.toFixed(0)})`)

        // Wait for expiry (60 seconds)
        console.log(`   Waiting 60s for result...`)
        await new Promise(r => setTimeout(r, 60000))

        // Get exit price
        const exitCandles = await fetchCandles(symbol, '1m', 1)
        const exitPrice = exitCandles[0].close

        // Determine result
        let result: 'WIN' | 'LOSS' | 'TIE'
        if (exitPrice === signal.entryPrice) {
          result = 'TIE'
        } else if (signal.direction === 'CALL') {
          result = exitPrice > signal.entryPrice ? 'WIN' : 'LOSS'
        } else {
          result = exitPrice < signal.entryPrice ? 'WIN' : 'LOSS'
        }

        const profit = result === 'WIN' ? amount * 0.92 : result === 'LOSS' ? -amount : 0
        balance += profit

        const icon = result === 'WIN' ? '🟢' : result === 'LOSS' ? '🔴' : '⚪'
        console.log(`${icon} Result: ${result} | Exit: $${exitPrice.toFixed(2)} | P/L: $${profit.toFixed(2)}`)
        console.log(`   Balance: $${balance.toFixed(2)}\n`)

        // Update signal result
        generator.updateSignalResult(signal.id, result === 'WIN' ? 'win' : 'loss')

        trades.push({
          timestamp: new Date().toISOString(),
          symbol,
          direction: signal.direction,
          strategy: signal.strategy,
          regime: regimeInfo.regime,
          confidence: signal.confidence,
          entryPrice: signal.entryPrice,
          exitPrice,
          result,
          profit,
        })
      }
    } catch (error) {
      console.error('Error:', error)
    }
  }

  const endTime = Date.now()
  const wins = trades.filter(t => t.result === 'WIN').length
  const losses = trades.filter(t => t.result === 'LOSS').length
  const netProfit = trades.reduce((sum, t) => sum + t.profit, 0)
  const avgConfidence = trades.length > 0
    ? trades.reduce((sum, t) => sum + t.confidence, 0) / trades.length
    : 0

  // 레짐별 분석
  const byRegime: Record<string, { trades: number; wins: number; losses: number; winRate: string }> = {}
  trades.forEach(t => {
    if (!byRegime[t.regime]) {
      byRegime[t.regime] = { trades: 0, wins: 0, losses: 0, winRate: '0%' }
    }
    byRegime[t.regime].trades++
    if (t.result === 'WIN') byRegime[t.regime].wins++
    else if (t.result === 'LOSS') byRegime[t.regime].losses++
  })
  Object.keys(byRegime).forEach(regime => {
    const r = byRegime[regime]
    r.winRate = r.trades > 0 ? `${((r.wins / r.trades) * 100).toFixed(1)}%` : '0%'
  })

  // 전략별 분석
  const byStrategy: Record<string, { trades: number; wins: number; losses: number; winRate: string }> = {}
  trades.forEach(t => {
    const stratKey = t.strategy.split(':')[0].trim()
    if (!byStrategy[stratKey]) {
      byStrategy[stratKey] = { trades: 0, wins: 0, losses: 0, winRate: '0%' }
    }
    byStrategy[stratKey].trades++
    if (t.result === 'WIN') byStrategy[stratKey].wins++
    else if (t.result === 'LOSS') byStrategy[stratKey].losses++
  })
  Object.keys(byStrategy).forEach(strat => {
    const s = byStrategy[strat]
    s.winRate = s.trades > 0 ? `${((s.wins / s.trades) * 100).toFixed(1)}%` : '0%'
  })

  const report: ForwardTestReport = {
    testInfo: {
      startTime: new Date(startTime).toISOString(),
      endTime: new Date(endTime).toISOString(),
      durationMinutes: Math.round((endTime - startTime) / 60000),
      symbol,
      initialBalance,
      riskPerTrade: `${riskPercent}%`,
      version: 'v2-high-winrate',
    },
    summary: {
      totalTrades: trades.length,
      wins,
      losses,
      winRate: trades.length > 0 ? `${((wins / trades.length) * 100).toFixed(1)}%` : '0%',
      netProfit: `$${netProfit.toFixed(2)}`,
      finalBalance: `$${balance.toFixed(2)}`,
      avgConfidence: `${(avgConfidence * 100).toFixed(1)}%`,
    },
    byRegime,
    byStrategy,
    trades,
  }

  // Save report
  const reportPath = './forward-test-results-v2.json'
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2))

  console.log('\n' + '═'.repeat(60))
  console.log('📊 FORWARD TEST V2 RESULTS')
  console.log('═'.repeat(60))
  console.log(`Duration: ${report.testInfo.durationMinutes} minutes`)
  console.log(`Total Trades: ${trades.length}`)
  console.log(`Win Rate: ${report.summary.winRate}`)
  console.log(`Avg Confidence: ${report.summary.avgConfidence}`)
  console.log(`Net P/L: ${report.summary.netProfit}`)
  console.log(`Final Balance: ${report.summary.finalBalance}`)
  console.log('')
  console.log('📈 By Regime:')
  Object.entries(byRegime).forEach(([regime, stats]) => {
    console.log(`   ${regime}: ${stats.winRate} (${stats.wins}/${stats.trades})`)
  })
  console.log('')
  console.log('🎯 By Strategy:')
  Object.entries(byStrategy).forEach(([strat, stats]) => {
    console.log(`   ${strat}: ${stats.winRate} (${stats.wins}/${stats.trades})`)
  })
  console.log('═'.repeat(60))
  console.log(`Results saved to: ${reportPath}`)

  return report
}

// Get duration from command line arg (default: 10 minutes for testing)
const durationArg = process.argv[2]
const durationMinutes = durationArg ? parseInt(durationArg) : 10
const durationMs = durationMinutes * 60 * 1000

console.log(`\n⏱️  Forward Test V2 configured for ${durationMinutes} minutes\n`)
runForwardTestV2(durationMs).catch(console.error)
