...
export async function initialize(ctx: ContentScriptContext): Promise<void> {
  ...
  try {
    ...
    ctx.signalGenerator = getSignalGeneratorV2({ symbols: ['CURRENT'], minConfidence: 0.3, expirySeconds: 60 })
    try { ctx.telegramService = await getTelegramService() } catch { console.warn('[PO] Telegram service unavailable, continuing without it'); ctx.telegramService = null }
    ...
    chrome.storage.onChanged.addListener((changes, areaName) => {
      if (areaName === 'local' && changes.appConfig) {
        ...
        if (JSON.stringify(oldTelegram) !== JSON.stringify(newTelegram)) {
          ...
          getTelegramService().then(svc => { ctx.telegramService = svc }).catch(() => {})
        }
      }
