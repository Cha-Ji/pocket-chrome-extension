...
import { handleMessage, getSystemStatus, registerMessageListener, validateConfigUpdate } from './message-handler'
...
describe('message-handler', () => {
  ...
  describe('SET_CONFIG_V2 with validation (P1-3)', () => {
    it('should reject invalid config and return validation errors', async () => {
      const result = await handleMessage(ctx, { type: 'SET_CONFIG_V2', payload: { tradeAmount: -1 } } as any) as any
    ...
    it('should apply valid config values after validation', async () => {
      const result = await handleMessage(ctx, { type: 'SET_CONFIG_V2', payload: { tradeAmount: 50, minPayout: 88 } } as any) as any
      expect(result.success).toBe(true)
