...
export class PayoutMonitor {
  ...
  async start(pollIntervalMs = 30000): Promise<void> {
    ...
    this.pollInterval = setInterval(async () => {
      try {
        ...
        log.error(
          `Interval error (${this.consecutiveErrors}/${PayoutMonitor.MAX_CONSECUTIVE_ERRORS}):`,
          error,
        );
  ...
  private markAssetUnavailable(assetName: string): void {
    ...
    if (existing) {
      ...
      this.unavailableAssets.set(assetName, {
        name: assetName,
        failedAt: Date.now(),
        retryCount: 1,
      });
  ...
  private findChartInactiveEl(): HTMLElement | null {
    ...
    const el = chart
      ...
      : (document.querySelector(
          '.chart-item .asset-inactive, .chart-block .asset-inactive',
        ) as HTMLElement | null);
  ...
  private isCurrentAsset(normalizedTarget: string): boolean {
    ...
    if (
      symbolEl &&
      this.normalizeAssetName(symbolEl.textContent || '').includes(normalizedTarget)
    ) {
      return true;
    }
    const pairEl = document.querySelector('.pair-number-wrap');
  ...
  private isItemInactive(item: Element): boolean {
    ...
    const inactiveChild = item.querySelector(
      '.inactive, .disabled, .closed, .locked, [data-status="closed"]',
    );
  ...
  private async findAssetElement(
    normalizedTarget: string,
    maxAttempts = 3,
    retryDelayMs = 800,
  ): Promise<{ element: HTMLElement; rawLabel: string; inactive?: boolean } | null> {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      for (const item of assetItems) {
        ...
        if (this.normalizeAssetName(rawLabel) === normalizedTarget) {
          ...
          const clickTarget =
            (item.querySelector('.alist__link') as HTMLElement) || (item as HTMLElement);
  ...
  async switchAsset(assetName: string): Promise<boolean> {
    ...
    if (found) {
      ...
      if (overlay) {
        ...
        // 리로드 완료 대기 (최대 5초)
        await this.waitForCondition(() => !this.findChartInactiveEl(), 5000, 500);
      }
  ...
  private scrapePayoutsFromDOM(): AssetPayout[] {
    ...
    assetItems.forEach((item) => {
      ...
      if (labelEl && profitEl) {
        const name = (labelEl.textContent || (labelEl as HTMLElement).innerText || '')
          .replace(/\u00a0/g, ' ')
          .replace(/\s+/g, ' ')
          .trim();
  ...
  private notifyObservers(): void {
    ...
    try {
      chrome.runtime
        .sendMessage({
          type: 'PAYOUT_UPDATE',
          payload: { highPayoutAssets: assets, totalAssets: this.assets.size },
        })
        .catch(() => {});
    } catch {}
  }
