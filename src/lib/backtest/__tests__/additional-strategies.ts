...
export function createWilliamsRStrategy(period: number = 14, oversold: number = -80, overbought: number = -20) {
  return {
...
export function createCCIStrategy(period: number = 20, oversold: number = -100, overbought: number = 100) {
  return {
    name: `CCI(${period})`,
...
export function createATRBreakoutStrategy(period: number = 14, multiplier: number = 1.5) {
  return {
    ...
    signal: (candles: Candle[], idx: number, data: { atr: number[]; sma: (number | null)[] }): 'CALL' | 'PUT' | null => {
...
export function createTripleStochStrategy(
  ...
  return {
    ...
    signal: (candles: Candle[], idx: number, data: TripleStochResult): 'CALL' | 'PUT' | null => {
      ...
      const allOversold = currShort.k < oversold + 10 && currMid.k < oversold + 15 && currLong.k < oversold + 20
      const shortGoldenCross = prevShort.k < prevShort.d && currShort.k >= currShort.d
      
      // All three in overbought zone + short dead cross
      const allOverbought = currShort.k > overbought - 10 && currMid.k > overbought - 15 && currLong.k > overbought - 20
...
export function createRSIDivergenceStrategy(period: number = 14, lookback: number = 10) {
  return {
    ...
    signal: (candles: Candle[], idx: number, data: { rsi: number[]; closes: number[] }): 'CALL' | 'PUT' | null => {
...
export function createEMACrossStrategy(fastPeriod: number = 9, slowPeriod: number = 21) {
  return {
    ...
    signal: (candles: Candle[], idx: number, data: { fast: number[]; slow: number[] }): 'CALL' | 'PUT' | null => {
