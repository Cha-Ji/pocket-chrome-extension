...
import { runFastBacktest, generateMediumTestData, formatStats, BacktestStats } from './test-helpers'
...
describe('SMMA Strategy Optimization', () => {
  ...
  it('should run full grid search', () => {
    ...
    results.slice(0, 10).forEach((r, i) => {
      const p = r.params
      console.log(`${i + 1}. TS=${p.trendStrength} OS=${p.oversold} EXP=${p.expiryCandles} | WR=${r.stats.winRate.toFixed(1)}% PF=${r.stats.profitFactor.toFixed(2)} $${r.stats.netProfit.toFixed(0)}`)
    })
