...
function recordIfWinning(
  result: { stats: { winRate: number; profitFactor: number; totalTrades: number; netProfit: number } },
  name: string,
  symbol: string,
  timeframe: string,
  regime: string,
...
describe('Strategy Collector', () => {
    ...
    for (const strategy of allStrategies) {
      try {
        ...
        if (result.stats.totalTrades > 0) {
          console.log(`${status} ${strategy.name.padEnd(30)} | ${String(result.stats.totalTrades).padStart(3)} | ${result.stats.winRate.toFixed(1).padStart(5)}% | $${result.stats.netProfit.toFixed(0).padStart(6)}`)
        }
  ...
  it('should test adaptive strategy across symbols', async () => {
    ...
    for (const symbol of symbols) {
      try {
        ...
        console.log(`${status} ${symbol} Adaptive | ${String(result.stats.totalTrades).padStart(3)} | ${result.stats.winRate.toFixed(1).padStart(5)}% | $${result.stats.netProfit.toFixed(0).padStart(6)}`)
  ...
  it('should output winning strategies summary', () => {
    ...
    if (winningStrategies.length === 0) {
      ...
      console.log('Strategy                       | Symbol   | TF  | Regime          | Trades |  Win% |     PF | Profit')
      ...
      console.log(`   Best win rate: ${winningStrategies[0]?.winRate.toFixed(1)}% (${winningStrategies[0]?.name})`)
