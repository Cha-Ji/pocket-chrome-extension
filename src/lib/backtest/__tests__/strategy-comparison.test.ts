  ...
  it('should find best strategy per market', () => {
    ...
    for (const market of markets) {
      ...
      const best = results.reduce((a, b) => 
        a.stats.profitFactor > b.stats.profitFactor ? a : b
      )
      
      const status = best.stats.winRate >= 53 ? '✅' : '❌'
      console.log(`${market.name}: ${best.name}`)
      console.log(`   → ${best.stats.totalTrades} trades | ${best.stats.winRate.toFixed(1)}% | PF ${best.stats.profitFactor.toFixed(2)} | $${best.stats.netProfit.toFixed(0)} ${status}\n`)
    }
...
describe('Combined Strategy Analysis', () => {
  it('should test across multiple market conditions', () => {
    ...
    aggregated.forEach((a, i) => {
      ...
      console.log(`${(i + 1).toString().padStart(2)}. ${a.name.padEnd(23)} | ${String(a.totalTrades).padStart(12)} | ${a.winRate.toFixed(1).padStart(9)}% | $${a.totalProfit.toFixed(0).padStart(10)} ${status}`)
    })
