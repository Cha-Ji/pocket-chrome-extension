...
function runTrendBacktest(
  ...
  ) => ReturnType<typeof adxDiCrossoverStrategy>,
  allCandles: Candle[],
  strategyName: string,
  payout: number = 0.92,
...
describe('Trend Following Strategies (v3)', () => {
  ...
  describe('ADX + DI Crossover Strategy', () => {
    ...
    it('should generate signals in strong downtrend', () => {
      ...
      console.log(
        `${result.strategy} [Strong Downtrend]: ${result.totalTrades} trades, ${result.winRate.toFixed(1)}% win rate`,
      );
  ...
  describe('Trend Pullback Strategy', () => {
    ...
    it('should perform well in weak downtrend', () => {
      ...
      console.log(
        `${result.strategy} [Weak Downtrend]: ${result.totalTrades} trades, ${result.winRate.toFixed(1)}% win rate`,
      );
  ...
  describe('Select Best Trend Strategy (Auto)', () => {
    it('should select appropriate strategy for market regime', () => {
      ...
      console.log(
        `  Strong Uptrend: ${strongResult.totalTrades} trades, ${strongResult.winRate.toFixed(1)}%`,
      );
      console.log(
        `  Weak Uptrend: ${weakResult.totalTrades} trades, ${weakResult.winRate.toFixed(1)}%`,
      );
      console.log(`  Ranging: ${rangingResult.totalTrades} trades (should be minimal)`);

      // 횡보장에서는 신호가 적어야 함
      expect(rangingResult.totalTrades).toBeLessThanOrEqual(
        strongResult.totalTrades + weakResult.totalTrades,
      );
    });
  });
  ...
  describe('52.1% Target Achievement in Trending Markets', () => {
    it('should find best strategy for strong uptrend', () => {
      ...
      strategies.forEach((s) => {
        ...
        console.log(
          `${status} | ${s.name.padEnd(15)} | ${result.totalTrades.toString().padStart(3)} trades | ${result.winRate.toFixed(1).padStart(5)}% | PF: ${result.profitFactor.toFixed(2)} | Conf: ${(result.avgConfidence * 100).toFixed(0)}%`,
        );
      });
    ...
    it('should find best strategy for weak trends', () => {
      ...
      const weakUpResult = runTrendBacktest(
        trendPullbackStrategy,
        weakUptrend,
        'Pullback (Weak Up)',
      );
      const weakDownResult = runTrendBacktest(
        trendPullbackStrategy,
        weakDowntrend,
        'Pullback (Weak Down)',
      );
      ...
      const status1 =
        weakUpResult.winRate >= 52.1 && weakUpResult.totalTrades >= 5 ? '✅ PASS' : '❌ FAIL';
      const status2 =
        weakDownResult.winRate >= 52.1 && weakDownResult.totalTrades >= 5 ? '✅ PASS' : '❌ FAIL';

      console.log(
        `${status1} | Weak Uptrend   | ${weakUpResult.totalTrades.toString().padStart(3)} trades | ${weakUpResult.winRate.toFixed(1).padStart(5)}%`,
      );
      console.log(
        `${status2} | Weak Downtrend | ${weakDownResult.totalTrades.toString().padStart(3)} trades | ${weakDownResult.winRate.toFixed(1).padStart(5)}%`,
      );
      console.log('='.repeat(70));
    });
    ...
    it('SUMMARY: Comprehensive trend strategy evaluation', () => {
      ...
      for (const market of markets) {
        ...
        for (const strategy of strategies) {
          ...
          console.log(
            `  ${status} ${strategy.name.padEnd(15)}: ${result.totalTrades.toString().padStart(3)} trades, ${result.winRate.toFixed(1).padStart(5)}% win rate`,
          );
        }
      ...
      for (const { market, results } of allResults) {
        ...
        if (validResults.length > 0) {
          ...
          console.log(
            `  ${status} ${market.padEnd(20)}: ${best.strategy} (${best.winRate.toFixed(1)}%, ${best.totalTrades} trades)`,
          );
        } else {
      ...
      const allBestResults = allResults
        .flatMap((ar) => ar.results)
