...
export function detectGaps(
  ...
  const interval = expectedIntervalMs > 0
    ? expectedIntervalMs
    : detectInterval(sortedCandles)
...
export function analyzeCandles(
  ...
  // Calculate coverage
  const timeSpanMs = deduped.length >= 2
    ? deduped[deduped.length - 1].timestamp - deduped[0].timestamp
    : 0
  const expectedCandles = timeSpanMs > 0 ? Math.round(timeSpanMs / interval) + 1 : deduped.length
  const coveragePercent = expectedCandles > 0
    ? (deduped.length / expectedCandles) * 100
    : 100
...
export function preprocessCandles(
  ...
  const interval = opts.expectedIntervalMs > 0
    ? opts.expectedIntervalMs
    : analysis.detectedIntervalMs
  ...
  switch (opts.gapStrategy) {
    case 'fill':
      ...
      if (analysis.totalMissingCandles > 0) {
        warnings.push(
          `Filled ${analysis.totalMissingCandles} candle(s) with linear interpolation`
        )
      }
      break
