...
for (const market of markets) {
  ...
  for (const cfg of configs) {
    ...
    console.log(
      `  ${cfg.name.padEnd(22)} | ${String(stats.total).padStart(3)} trades | Win ${stats.winRate.padStart(5)}% | PF ${stats.profitFactor.padStart(5)} | $${stats.netProfit.padStart(6)} ${status}`,
    );
  }
}
