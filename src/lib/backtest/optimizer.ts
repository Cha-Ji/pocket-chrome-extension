...
export async function runFullOptimization(
  ...
  if (bestOverall) {
    ...
    recommendations.push(`Optimal params: ${JSON.stringify(bestOverall.bestParams)}`);
    recommendations.push(
  ...
  if (profitableStrategies.length === 0) {
    ...
    recommendations.push(
      'Consider: Lower timeframe, different indicators, or market condition filters',
    );
  } else {
...
export function bayesianOptimize(
  ...
  for (let i = initialSamples; i < maxIterations; i++) {
    ...
    const evaluation = evaluateParams(
      engine,
      config,
      candles,
      bestNextParams,
  ...
  if (verbose) {
    console.log(
      `[Bayesian] Optimization complete. Best score: ${results[0]?.score.toFixed(2) ?? 'N/A'}`,
    );
  }
...
export function geneticOptimize(
  ...
  for (let gen = 1; gen <= generations; gen++) {
    ...
    if (verbose && gen % 5 === 0) {
      console.log(
        `[Genetic] Generation ${gen}: Best score ${population[0]?.score.toFixed(2) ?? 'N/A'}`,
      );
    }
  }
  ...
  if (verbose) {
    console.log(
      `[Genetic] Evolution complete. Best score: ${population[0]?.score.toFixed(2) ?? 'N/A'}`,
    );
  }

  // Return all valid results sorted by score
  return allResults.filter((r) => r.score > -Infinity).sort((a, b) => b.score - a.score);
}
...
export function gridOptimize(
  ...
  const { objective = 'netProfit', minTrades = 5, maxCombinations, verbose = false } = options;
  ...
  const limited = maxCombinations ? unifiedResults.slice(0, maxCombinations) : unifiedResults;
