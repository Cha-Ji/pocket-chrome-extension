...
async function main() {
  ...
  console.log(
    `${'STRATEGY'.padEnd(15)} | ${'TRADES'.padStart(6)} | ${'WINRATE'.padStart(8)} | ${'PF'.padStart(5)} | ${'PROFIT'.padStart(8)}`,
  );
  console.log('-'.repeat(70));
  ...
  allStrategies.forEach((s) => {
    ...
    console.log(
      `${status} ${s.name.padEnd(13)} | ${result.totalTrades.toString().padStart(6)} | ${result.winRate.toFixed(2).padStart(7)}% | ${result.profitFactor.toFixed(2).padStart(5)} | $${result.netProfit.toFixed(1).padStart(7)}`,
    );
  });
  ...
  if (passing.length > 0) {
    ...
    passing
      ...
      .forEach((r) => {
        console.log(
          `   🏆 ${r.strategy}: ${r.winRate.toFixed(2)}% (${r.totalTrades} trades, PF: ${r.profitFactor.toFixed(2)})`,
        );
      });
  } else {
    ...
    for (const rsiP of [7, 14]) {
      for (const oversold of [20, 25, 30]) {
        ...
        if (res.winRate >= 52.1 && res.totalTrades > 20) {
          console.log(
            `   ✨ FOUND: ${res.strategy} -> ${res.winRate.toFixed(2)}% (${res.totalTrades} trades)`,
          );
        }
      }
    }
