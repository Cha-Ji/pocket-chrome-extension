...
export const DEFAULT_SCORE_WEIGHTS: ScoreWeights = {
  winRate: 0.3, // 승률 (핵심 — 52.1% 이상이어야 수익)
  expectedValue: 0.2, // 기대값 (EV > 0 필수)
  maxDrawdown: 0.15, // 최대 드로다운 (낮을수록 좋음)
  maxLosingStreak: 0.1, // 최대 연속 손실 (낮을수록 좋음)
  profitFactor: 0.1, // 이익 팩터 (높을수록 좋음)
  tradeCount: 0.1, // 거래 횟수 (충분한 표본)
...
function scoreExpectedValue(ev: number): number {
  ...
  if (ev <= 0.1) return linearMap(ev, 0.02, 0.1, 50, 100);
  return 100;
}
...
function scoreTradeCount(count: number): number {
  ...
  if (count < SCORING_THRESHOLDS.excellentTradeCount) {
    return linearMap(
      count,
      SCORING_THRESHOLDS.minTradesForSignificance,
      SCORING_THRESHOLDS.excellentTradeCount,
      30,
      100,
    );
  }
