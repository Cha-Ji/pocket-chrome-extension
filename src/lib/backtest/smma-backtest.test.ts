  ...
  describe('Backtest Execution', () => {
    ...
    it('should test in different market conditions', () => {
      ...
      results.forEach((r) => {
        ...
        console.log(
          `${profitable} ${r.condition.padEnd(10)}: ${r.winRate.toFixed(1)}% win rate, ${r.trades} trades, $${r.profit.toFixed(2)} profit`,
        );
      });
      console.log('═══════════════════════════════════════');
    });
    ...
    it('should compare with aggressive version', () => {
      ...
      for (const c of configs) {
        ...
        console.log(
          `${profitable} ${c.name}: ${result.winRate.toFixed(1)}% (${result.totalTrades} trades, $${result.netProfit.toFixed(2)})`,
        );
      }
    });
  });
