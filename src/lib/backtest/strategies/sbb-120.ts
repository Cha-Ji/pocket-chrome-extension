...
export function sbb120Strategy(
  ...
  // ── 2. Bandwidth 시리즈 계산 ─────────────────────────────
  const bandwidths = bbArray.map((bb) => (bb.middle > 0 ? (bb.upper - bb.lower) / bb.middle : 0));
  ...
  const percentileIndex = Math.max(
    0,
    Math.floor((sortedBWs.length * cfg.squeezePercentile) / 100) - 1,
  );
