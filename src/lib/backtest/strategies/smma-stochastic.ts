...
export const SMMAStochasticStrategy: Strategy = {
  ...
  generateSignal(candles: Candle[], params: Record<string, number>): StrategySignal | null {
    const { stochK, stochD, stochSmooth, oversold, overbought, trendStrength, overlapTolerance } = params
    ...
    const overlapAmount = hasOverlap ? Math.min(shortMax, longMax) - Math.max(shortMin, longMin) : 0
    ...
    if (isUptrend && goldenCross && fromOversold) {
      direction = 'CALL'
      confidence = Math.min(1, (shortAboveLongCount / 6) * 0.5 + (oversold - Math.min(prevStoch.k, currentStoch.k)) / 40 * 0.5)
    ...
    // PUT: Downtrend + Stochastic dead cross from overbought
      ...
      direction = 'PUT'
      confidence = Math.min(1, (shortBelowLongCount / 6) * 0.5 + (Math.max(prevStoch.k, currentStoch.k) - overbought) / 40 * 0.5)
