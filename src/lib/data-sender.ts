    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          ...
          log.data(`Bulk send network error (attempt ${attempt + 1}/${maxRetries + 1}), retrying in ${backoffMs}ms...`)
          await new Promise(r => setTimeout(r, backoffMs))
        } else {
          senderStats.bulkFailCount++
          log.fail(`Bulk send network error after ${maxRetries + 1} attempts: ${error instanceof Error ? error.message : String(error)}`)
        }
      }
    }
  },
