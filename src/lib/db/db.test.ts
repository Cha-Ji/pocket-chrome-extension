...
import { db, TickRepository, StrategyRepository, SessionRepository, TradeRepository } from './index'
...
describe('Database Operations', () => {
  ...
  describe('TickRepository', () => {
    ...
    it('should bulk add ticks', async () => {
      const now = Date.now()
      const ticks = [
        { ticker: 'EURUSD', timestamp: now - 2000, price: 1.0850 },
    ...
    it('should get ticks by ticker', async () => {
      const now = Date.now()
      await TickRepository.add({ ticker: 'EURUSD', timestamp: now, price: 1.0850 })
      await TickRepository.add({ ticker: 'GBPUSD', timestamp: now + 1, price: 1.2650 })
    ...
    it('should delete old ticks', async () => {
      ...
      await TickRepository.add({ ticker: 'EURUSD', timestamp: oldTimestamp, price: 1.0850 })
  ...
  describe('TradeRepository', () => {
    it('should create and finalize a trade', async () => {
      ...
      // Finalize trade
      const result = await TradeRepository.finalize(tradeId, 1.0860, 'WIN', 85)
      expect(result.updated).toBe(true)
      expect(result.trade?.result).toBe('WIN')
      expect(result.trade?.profit).toBe(85)
      expect(result.trade?.exitPrice).toBe(1.0860)
      ...
      expect(trade?.exitPrice).toBe(1.0860)
    })
    ...
    it('should be idempotent — second finalize returns updated=false', async () => {
      ...
      // First finalize
      const first = await TradeRepository.finalize(tradeId, 1.0860, 'WIN', 85)
      expect(first.updated).toBe(true)

      // Second finalize — should be no-op
      const second = await TradeRepository.finalize(tradeId, 1.0870, 'LOSS', -100)
    ...
    it('should not double-count session stats on duplicate finalize', async () => {
      ...
      // Finalize twice
      await TradeRepository.finalize(tradeId, 1.0860, 'WIN', 85)
      await TradeRepository.finalize(tradeId, 1.0860, 'WIN', 85)
    ...
    it('should correctly track TIE in session stats', async () => {
      ...
      // Finalize as TIE
      const result = await TradeRepository.finalize(tradeId, 1.0850, 'TIE', 0)
      expect(result.updated).toBe(true)
    ...
    it('should return updated=false for non-existent trade', async () => {
      const result = await TradeRepository.finalize(99999, 1.0860, 'WIN', 85)
