...
export class PocketOptionExecutor implements IExecutor {
  ...
  async setExpiration(seconds: number): Promise<void> {
    ...
    console.warn(
      `[PO-Executor] setExpiration(${seconds}s) - manual setup required`,
    )
  }
  ...
  async switchAsset(assetName: string): Promise<boolean> {
    const normalizedTarget = assetName
      .replace(/\s+/g, ' ')
      .trim()
      .toLowerCase()
    ...
    for (const item of items) {
      ...
      if (text === normalizedTarget) {
        const link =
          (item.querySelector('.alist__link') as HTMLElement) ||
          (item as HTMLElement)
        this.simulateClick(link)
        await this.wait(2000)

        // 전환 확인
        const afterEl = document.querySelector(
          PO_PAYOUT_SELECTORS.pairTrigger,
        )
        return (
          afterEl?.textContent?.toLowerCase().includes(normalizedTarget) ??
          false
        )
      }
    }
  ...
  private resolveElement(key: string): HTMLElement | null {
    ...
    // 기본 셀렉터
    const defaultSel = (PO_DEMO_SELECTORS as Record<string, string>)[key]
    if (defaultSel) {
