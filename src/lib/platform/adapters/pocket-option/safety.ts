...
export class PocketOptionSafetyGuard implements ISafetyGuard {
  ...
  getAccountType(): { type: AccountType; confidence: Confidence } {
    ...
    const hasChartDemoClass = !!document.querySelector(PO_DEMO_DETECTION.demoChartClass);
    const labelText =
      document.querySelector(PO_DEMO_DETECTION.balanceLabel)?.textContent?.toLowerCase() || '';
  ...
  onAccountTypeChange(callback: (type: AccountType) => void): Unsubscribe {
    ...
    return () => {
      this.accountTypeCallbacks = this.accountTypeCallbacks.filter((cb) => cb !== callback);
    };
  }
