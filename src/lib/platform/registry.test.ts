...
function createMockDetector(
  ...
  return {
    ...
    createAdapter: vi.fn().mockReturnValue(
      adapter ?? createMockAdapter(platformId, platformId),
    ),
  }
}
...
describe('PlatformRegistry', () => {
  ...
  describe('detectAndInitialize', () => {
    it('should detect, create, and initialize adapter', async () => {
      ...
      const result = await registry.detectAndInitialize(
        'https://example.com',
        document,
      )
    ...
    it('should return null when initialization fails', async () => {
      const adapter = createMockAdapter('test', 'Test')
      ;(adapter.initialize as ReturnType<typeof vi.fn>).mockRejectedValue(
