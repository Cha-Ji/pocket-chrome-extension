...
export class DemoRunner {
  ...
  async start(symbols: string[] = ['BTCUSDT'], intervalMs: number = 10000): Promise<void> {
...
export async function runCliDemo(durationMs: number = 60000): Promise<Signal[]> {
  ...
  runner.onUpdate((update) => {
    ...
    const regimeIcon = update.regime.includes('up')
      ? '📈'
      : update.regime.includes('down')
        ? '📉'
        : '↔️';
    console.log(
      `[${time}] ${update.symbol} $${update.price.toFixed(2)} | ${regimeIcon} ${update.regime} (ADX: ${update.adx.toFixed(1)})`,
    );
  });
