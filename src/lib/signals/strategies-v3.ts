...
export class StrategyV3 implements Strategy {
  ...
  generateSignal(candles: Candle[], params: Record<string, number>): StrategySignal | null {
    ...
    const tr = highs.map((h, i) => Math.max(h - lows[i], Math.abs(h - (closes[i-1] || h)), Math.abs(lows[i] - (closes[i-1] || lows[i]))))
    ...
    if (isTrendingUp) {
      ...
      if (currentPrice <= sFast && currentPrice >= sMid && 
          stoch.k < 35 && stoch.k > stoch.d && rsi > 48) {
        direction = 'CALL'
        confidence = 0.9
      ...
      if (currentPrice >= sFast && currentPrice <= sMid && 
          stoch.k > 65 && stoch.k < stoch.d && rsi < 52) {
        direction = 'PUT'
        confidence = 0.9
      ...
      } else if (currentPrice >= recentHigh * 0.9999 && stoch.k > 85 && stoch.k < stoch.d && rsi > 75) {
        direction = 'PUT'
        confidence = 0.8
