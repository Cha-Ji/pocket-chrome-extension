...
export function rsiSignal(candles: Candle[], period = 14, oversold = 35, overbought = 65): 'CALL' | 'PUT' | null {
  const closes = candles.map(c => c.close)
...
export function getBestStrategies(regime: MarketRegime): StrategyConfig[] {
  return WINNING_STRATEGIES.filter(s => s.bestRegimes.includes(regime))
