...
describe('Strategy RAG', () => {
  ...
  describe('searchStrategies', () => {
    beforeEach(() => {
      addStrategy({
        ...
        indicators: ['RSI'],
        conditions: [
          { type: 'entry', direction: 'CALL', description: 'RSI oversold', indicators: [] },
        ],
      });
      addStrategy({
        ...
        indicators: ['MACD'],
        conditions: [
          { type: 'entry', direction: 'CALL', description: 'MACD cross up', indicators: [] },
        ],
      });
      addStrategy({
        title: 'RSI + BB Combo',
  ...
  describe('parseYouTubeStrategy', () => {
    ...
    it('유효하지 않은 URL은 에러를 던진다', async () => {
      await expect(parseYouTubeStrategy('https://example.com')).rejects.toThrow(
        'Invalid YouTube URL',
      );
    });
  });
