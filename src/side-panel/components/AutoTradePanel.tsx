  ...
  return (
    <div className="bg-pocket-dark rounded-lg p-4 space-y-4">
      ...
      {stats.isHalted && (
        <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-3">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => { trader.resume(); setStats(trader.getStats()) }}
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="bg-pocket-darker rounded-lg p-2 text-center">
          <div className={`text-lg font-bold ${parseFloat(winRate) >= 53 ? 'text-green-400' : 'text-red-400'}`}>
            {winRate}%
          </div>
      ...
      {(stats.consecutiveLosses >= 2 || stats.consecutiveWins >= 2) && (
        <div className={`text-center text-sm py-1 rounded ${
          stats.consecutiveWins >= 2 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
        }`}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              ...
              <input
                ...
                onChange={(e) => setConfig({ ...config, maxDrawdown: parseFloat(e.target.value) || 20 })}
                className="w-full mt-1 bg-pocket-darker text-white text-sm rounded px-3 py-2 border border-gray-600"
              />
            </div>
            <div className="flex-1">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="rounded"
            />
            <label htmlFor="demoMode" className="text-sm text-gray-300">Demo Mode (시뮬레이션)</label>
          </div>
        </div>
      )}
