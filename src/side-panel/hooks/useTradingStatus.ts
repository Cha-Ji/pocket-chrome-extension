...
export function useTradingStatus() {
  ...
  const startTrading = useCallback(async (): Promise<{ success: boolean; error?: string }> => {
    setIsLoading(true)
    try {
      const response = await sendRuntimeMessage('START_TRADING') as { success: boolean; error?: string } | null
  ...
  const stopTrading = useCallback(async (): Promise<{ success: boolean; error?: string }> => {
    setIsLoading(true)
    try {
      const response = await sendRuntimeMessage('STOP_TRADING') as { success: boolean; error?: string } | null
      return response ?? { success: false, error: 'No response' }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error'
